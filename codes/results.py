...
def complexity(x, y, z, model_name, min_deg=1, max_deg=10, alpha = 0):
    """
...
def generate_heatmaps(x,y,z,model_name,min_deg=1,max_deg=10,num_alpha=13,min_alpha=-10,max_alpha=2):
    ...
    alpha_vals = np.linspace(min_alpha,max_alpha,num_alpha)
    degrees = np.linspace(min_deg, max_deg, max_deg - min_deg + 1)
    variables = pd.DataFrame(columns=['degrees', 'mse', 'log lambda'])
    ...
    doc_matrix = doc_raw.pivot("degrees","log lambda","mse")
